            ...
            if (hunted.team != -1) {
                if (params.getIntegerArgument(1) == 0) {
                    if (player.bountySet != 0) {
                        ...
                        player.bountyActive = false;
                        World.getWorld().broadcast("- " + player.getColoredName() + " &ehas removed their bounty on " + hunted.getColoredName());
                    } else {
                        player.getActionSender().sendChatMessage("You do not have an active bounty on this player!");
                    }
                }
                else {
                    if(hunted != null && params.getIntegerArgument(1) >= 50) {
                        if (params.getIntegerArgument(1) <= player.getStorePoints()) {
                            if (player.bountyActive == true) {
                                player.getActionSender().sendChatMessage("You currently have an active bounty on " + hunted.getColoredName());
                                player.getActionSender().sendChatMessage("Use \"/bounty " + hunted.getName() + " 0\" to remove it");
                            }
                            else {
                                if (hunted.bountyMode == true) {
                                    ...
                                    World.getWorld().broadcast("- " + player.getColoredName() + " &ehas added " + params.getIntegerArgument(1) + " on " + hunted.getColoredName() + "&e's bounty! The bounty is now " + hunted.bountyAmount);
                                }
                                else {
                                    World.getWorld().broadcast("- " + player.getColoredName() + " &ehas set a bounty of " + params.getIntegerArgument(1) + " on " + hunted.getColoredName() + "! &4KILL &ethem five times to collect the bounty.");
                                    player.bountySet = params.getIntegerArgument(1);
                                    hunted.bountied = hunted;
